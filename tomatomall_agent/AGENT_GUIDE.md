...
| 功能                  | 实现方式                                                                                                                                                                             | 代码位置                                                                                                                   |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| **ReAct 推理循环**    | LangChain v1 `create_agent` + 中间件架构（非已弃用的 `create_react_agent`）                                                                                                          | [shopping_agent.py](file:///d:/Develop/软工2/TomatoMall2.0/nju_serverend_dev/tomatomall_agent/src/agent/shopping_agent.py) |
| **多轮对话记忆**      | LangGraph Checkpointer（MemorySaver），通过 `conversation_id` 隔离会话                                                                                                               | [chat_memory.py](file:///d:/Develop/软工2/TomatoMall2.0/nju_serverend_dev/tomatomall_agent/src/memory/chat_memory.py)      |
| **多维度商品搜索**    | `search_products` 支持 keyword / max_price / min_price / min_rate / sort_by，keyword 在 title+description+detail 三字段匹配                                                          | [product_tool.py](file:///d:/Develop/软工2/TomatoMall2.0/nju_serverend_dev/tomatomall_agent/src/tool/product_tool.py)      |
| **Human-in-the-loop** | `HumanInTheLoopMiddleware` 框架级 HITL：`add_to_cart` 调用时中间件自动 `interrupt` 暂停图执行，用户通过 `/agent/resume` 用 `Command(resume={"decisions":[{"type":"approve"}]})` 恢复 | [shopping_agent.py](file:///d:/Develop/软工2/TomatoMall2.0/nju_serverend_dev/tomatomall_agent/src/agent/shopping_agent.py) |
| **引用溯源**          | 工具返回嵌入 `[ID:X]`，响应中提取 `references` 商品ID列表                                                                                                                            | [product_tool.py](file:///d:/Develop/软工2/TomatoMall2.0/nju_serverend_dev/tomatomall_agent/src/tool/product_tool.py)      |
